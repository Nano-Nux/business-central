...
export default function RolesPage() {
  ...
  return <AdminShell session={session} active="roles">
    <header className="topbar"><div><p className="eyebrow">ROLE-BASED ACCESS</p><h1>User Roles</h1><p className="muted">Manage merchant roles and the backend permissions each role grants.</p></div><button className="outline-button" disabled={busy} onClick={() => { if (!merchantID) { setError("Select a merchant before adding a role."); return; } setError(""); setEditing(null); setForm(emptyForm); setShowForm(true); }}>+ Add role</button></header>
