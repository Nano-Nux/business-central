...
- Add User defaults to the Merchant account role. Merchant mode collects merchant details and a POS complexity level (SIMPLE by default, COMPLEX, or MINI) and calls `POST /api/v1/admin/merchant-users`; other roles require an existing merchant and show `Please create a merchant first.` when none exists.
