...
CLOUDFLARE_TUNNEL_NAME=${CLOUDFLARE_TUNNEL_NAME:-business-central}
# Nginx is the single local entry point. It routes each hostname to the
# backend or SeaweedFS service, so Cloudflare should target Nginx.
CLOUDFLARE_ORIGIN_PORT=${CLOUDFLARE_ORIGIN_PORT:-5000}
API_ORIGIN_SERVICE=${API_ORIGIN_SERVICE:-http://127.0.0.1:$CLOUDFLARE_ORIGIN_PORT}
FILES_ORIGIN_SERVICE=${FILES_ORIGIN_SERVICE:-http://127.0.0.1:$CLOUDFLARE_ORIGIN_PORT}
