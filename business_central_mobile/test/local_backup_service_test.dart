...
void main() {
  ...
  test('local backup round-trips merchant-scoped operational catalog data', () async {
    // Insert product, variant, order, order lines
    await database.into(database.cachedCatalogProducts).insert(
    ...
    await database.into(database.cachedCatalogVariants).insert(
    ...
    await database.into(database.localOrders).insert(
    ...
    await database.into(database.localOrderLines).insert(
    ...
    expect(await database.select(database.cachedCatalogProducts).get(), isEmpty);
    ...
    final restoredProducts = await database.select(database.cachedCatalogProducts).get();
    expect(restoredProducts, hasLength(1));
    expect(restoredProducts.first.name, 'Cable');

    final restoredVariants = await database.select(database.cachedCatalogVariants).get();
    ...
    final restoredLines = await database.select(database.localOrderLines).get();
    expect(restoredLines, hasLength(1));
    expect(restoredLines.first.unitPrice, '5.00');
  });

  test('local backup rejects a cross-merchant restore before writing', () async {
    final backup = LocalBackupService(database);
    final payload = await backup.exportMerchant(merchantId: merchantId);

    expect(
      () => backup.restoreMerchant(merchantId: 'different-merchant', payload: payload),
  ...
  test('local backup rejects tampered payloads by checksum', () async {
    ...
    expect(
      () => backup.restoreMerchant(merchantId: merchantId, payload: tamperedPayload),
      throwsA(isA<LocalBackupException>()),
    );
  });
}
