...
void main() {
  ...
  test('uploadBackup sends exported merchant data with checksum and device id', () async {
    ...
    expect(capturedOptions!.path, 'https://api.example.com/api/v1/merchants/m1/backups');
  ...
  test('restoreLatestBackup downloads and restores into local database', () async {
    ...
    final merchant = await (database.select(database.merchants)..where((t) => t.id.equals('m1'))).getSingle();
    expect(merchant.name, 'Test Merchant');
    expect(merchant.currencyCode, 'USD');

    final shops = await (database.select(database.shops)..where((t) => t.merchantId.equals('m1'))).get();
