...
| Products | Cached catalog; basic create/update/delete can queue with stable local IDs and optimistic versions | `CATALOG_PRODUCT` create/update/delete, version conflict, tombstone publication, and explicit delete rejection for referenced products | Typed lifecycle operations with local projections, dependency ordering, and restart-safe outbox | Catalog offline tests | B-U, P-U | Partial — variants, SKUs, images, pricing, and category CRUD remain online-only |
| Variants and SKUs | Cached reads support POS; create/update/delete persist locally and synchronize after parent products | `CATALOG_VARIANT` create/update/delete, merchant scope, optimistic versions, dependency validation, direct publication, and visible reference rejection | Durable variant projection/outbox with parent-product dependency ordering and tombstones | Variant queue/dependency tests | B-U, P-U | Partial — prices, inventory policy, images, and stock mutations remain separate |
...
| Repair images, parts, status, approvals, warranties | Cached ticket readable; mutations blocked | Canonical lifecycle/stock APIs only | Explicit online-only controls; no generic queue | Connected tests only | B-U, P-U | Complete for current portal surface (future policies remain online-only) |
